...
pub fn build_active_inventory_analysis_report_with_provider_current(
    ...
    let applicability = evaluate_provider_current(&inventory, provider_current)?;
    ...
    match applicability.status {
        ApplicabilityStatus::Applies => {
            ...
            analysis.claims.push(Claim::new(
                ClaimKind::Derived,
                format!(
                    "The supplied provider-current context matches frozen current work `{}` at head `{}`.",
...
fn gated_analysis(
    ...
    let evidence = Evidence::new(format!(
        "Frozen provider work coordinate: work=`{}` head=`{}`; supplied provider-current coordinate: work=`{}` head=`{actual_head}`.",
        inventory.current.id,
        inventory.current.head_sha,
