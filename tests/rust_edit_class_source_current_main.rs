...
fn focused_source_values_and_focus_admission_survive_current_main() {
    ...
    let syntax_result = collect_rust_edit_class_source(&repo.0, &subject(&syntax)).unwrap();
    assert_eq!(
        syntax_result.observation.value_state,
        DiscriminatorValueState::Known { value_ref: "syntax_changed".to_string() }
    );
    assert_eq!(
    ...
    let comments_result = collect_rust_edit_class_source(&repo.0, &subject(&comments)).unwrap();
    assert_eq!(
        comments_result.observation.value_state,
        DiscriminatorValueState::Known { value_ref: "comments_or_docs_only".to_string() }
    );

    git(&repo.0, &["checkout", "-q", &root]);
    let root_result = collect_rust_edit_class_source(&repo.0, &subject(&root)).unwrap();
    assert!(matches!(root_result.observation.value_state, DiscriminatorValueState::Unknown { .. }));

    git(&repo.0, &["checkout", "-q", "master"]);
    let unrelated = repo.commit_unrelated();
    let unrelated_result = collect_rust_edit_class_source(&repo.0, &subject(&unrelated)).unwrap();
    assert!(matches!(
...
fn source_bridge_uses_v2_consumption_applicability_and_closes_missing_frontier() {
    ...
    let source = collect_rust_edit_class_source(&repo.0, &subject(&comments)).unwrap();
    ...
    assert_eq!(plan.evidence_plan.unwrap().status, EvidencePlanStatus::Selected);
    ...
    assert_eq!(evaluation.frontiers[0].status, ObservationFrontierStatus::Current);
    assert_eq!(evaluation.frontiers[0].current[0].value_ref, "comments_or_docs_only");
}

#[test]
fn moved_checkout_and_bad_subject_coordinates_fail_closed() {
    let (repo, _root, syntax, comments) = seeded();
    let moved = collect_rust_edit_class_source(&repo.0, &subject(&syntax)).unwrap();
    assert_eq!(moved.observation.applicability.status, ObservationApplicabilityStatus::Invalid);
    assert_eq!(git_text(&repo.0, &["rev-parse", "HEAD"]), comments);

    let mut bad_path = subject(&comments);
    bad_path.path = "../src/lib.rs".to_string();
    assert!(collect_rust_edit_class_source(&repo.0, &bad_path).is_err());

    let mut short_revision = subject(&comments);
    short_revision.revision = "deadbeef".to_string();
    assert!(collect_rust_edit_class_source(&repo.0, &short_revision).is_err());
}
