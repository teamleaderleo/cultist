...
fn context(work_id: &str, head_sha: Option<&str>) -> ProviderCurrentWorkContext {
...
fn matching_provider_current_coordinate_preserves_normal_collision_analysis() {
    ...
    assert!(report.claims.iter().any(|claim| {
        claim.kind == ClaimKind::Derived
            && claim
                .message
                .contains("provider-current context matches frozen current work")
    }));
...
fn moved_provider_current_head_gates_strong_collision() {
    ...
    let finding = report
        ...
        .unwrap();
    assert!(finding.claims.iter().any(|claim| claim.kind == ClaimKind::Derived));
    assert!(finding.question.as_deref().is_some_and(|question| {
        question.contains("Refresh the active-work inventory")
    }));
    fs::remove_dir_all(root).unwrap();
}
...
fn unavailable_provider_current_head_preserves_unknown_and_gates_collision() {
    ...
    let finding = report
        ...
        .unwrap();
    assert!(finding.claims.iter().any(|claim| claim.kind == ClaimKind::Unknown));
    assert!(finding.claims.iter().flat_map(|claim| &claim.evidence).any(|evidence| {
        evidence.message.contains("head=`<unavailable>`")
    }));
    fs::remove_dir_all(root).unwrap();
}
