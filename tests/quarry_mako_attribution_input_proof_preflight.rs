...
#[test]
fn quarry_mako_attribution_input_proof_is_path_quiet_and_semantically_unknown() {
    let root = std::env::temp_dir().join(format!(
        "cultist-quarry-mako-attribution-input-root-{}",
        std::process::id()
    ));
    let inventory_path = std::env::temp_dir().join(format!(
        "cultist-quarry-mako-attribution-input-inventory-{}.json",
        std::process::id()
    ));
    ...
    let inventory = r#"{
  "schema_version": 1,
  "source": "github:bounded-performance-family-mako-2026-08-22T20:03:00Z",
  "observed_at": "2026-08-22T20:03:00Z",
  "current": {
    "id": "mako/quarry-attribution-input-equivalence-proof",
    "kind": "planned_profiling",
    "title": "Mako prove alternate attribution diagnostic-source replay inputs",
    ...
    "changed_paths": [
      ".github/workflows/research-563-attribution-source-input-equivalence.yml",
      "scripts/research_563_attribution_source_input_equivalence.py"
    ]
  },
  "active_work": [
    {
