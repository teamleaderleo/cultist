...
#[test]
fn quarry_mako_direct_timing_composition_is_path_quiet() {
    let root = std::env::temp_dir().join(format!(
        "cultist-quarry-mako-direct-timing-root-{}",
        std::process::id()
    ));
    let inventory_path = std::env::temp_dir().join(format!(
        "cultist-quarry-mako-direct-timing-inventory-{}.json",
        std::process::id()
    ));
    ...
    let inventory = r#"{
  "schema_version": 1,
  "source": "github:bounded-performance-family-mako-2026-08-22T20:37:00Z",
  "observed_at": "2026-08-22T20:37:00Z",
  "current": {
    "id": "mako/direct-timing-composition",
    "kind": "planned_work",
    "title": "Mako compose accepted direct immutable timing into best alpha carrier",
    "url": "https://github.com/Coreys-Quarry/quarry/issues/563",
    "head_ref": "main",
    "head_sha": "4da48af927d0ca389dea8a1d6a12085e25303259",
    "updated_at": "2026-08-22T20:37:00Z",
    "draft": false,
    "activity": "preparation",
    "changed_paths": [
      ".github/workflows/research-563-combined-alpha-direct-timing.yml",
  ...
  "coordination_edges": []
    ...
    assert!(
        overlaps.is_empty(),
        "direct timing composition unexpectedly overlaps active work: {stdout}"
    );
