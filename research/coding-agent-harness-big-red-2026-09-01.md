...
### Pi dogfood and automatic compaction

**PROVEN:** Three bounded dogfood cycles converted recurrent orchestration defects into canonical fixes. Stensibly `cb7bb66b` automatically mounts linked-worktree Git common metadata read-only; `04a6a7cc` corrects Pi usage algebra (`input` is uncached, `cacheRead` is cached, and reasoning is a breakdown rather than an additive total); and `2241e53b` separates durable Pi session state from immutable attempt evidence. Atlas `f440a81` makes the compaction listener select the active runtime that observed the root thread instead of a newer nested rootless proxy.

**PROVEN:** A physical two-process Pi resume used distinct immutable attempt directories with one explicit session directory and session ID. The second process reproduced exact evidence and failed-approach identities without repository reads, credential copying, transcript reconstruction, or edits to the first receipt. The session transcript, Git worktree, and receipts—not a live Pi process—were sufficient for this ordinary restart treatment. This is not forced-kill equivalence.

**OBSERVED:** Pi twice exited 0 after making the intended implementation edits without emitting the required `luna_result`; each attempt was correctly classified `harness_failed`. One attempt emitted 4.5 MB of JSON events for a three-file change, and commander review found an accidentally deleted security-boundary paragraph plus missing overlap/symlink cases. This preserves the case for bounded receipts plus independent diff/tests rather than trusting worker narration or exit status.

**PROVEN:** The current Codex root physically compacted six times. Compaction began at 85.6–99.0% of the effective total-input window (median 91.8%) and reduced measured input by 86.7–89.9% (median 88.1%). The installed Big Red configuration has no explicit auto-compact token limit, so the pinned Codex source falls back to the model hard-window boundary under `body_after_prefix`. Content-free activity evidence SHA-256: `b417eecb6ff9d6bc811efef3cd0ca79d30b7b3ca58261d6eafd62c86fc62d709`.

**PROVEN:** Pi 0.84.4 defaults to `contextTokens > contextWindow - 16,384`, keeps approximately 20,000 recent tokens, persists a structured summary plus `firstKeptEntryId`, and rebuilds summary-plus-kept context on resume. In a forced-threshold physical probe, seven fresh Pi processes produced exactly two automatic `threshold` compactions. They reduced estimated active context from 106,696 to 51,101 tokens and from 107,491 to 51,253 tokens. After the second compaction and another process restart, Pi reproduced the exact first evidence identity, failed-approach identity, and second evidence identity without repository reads or manual reconstruction. Session SHA-256: `a72d6f6487da55bd1ad8eeb5887114b51040f1c6b1d115d10574bc89ba88350e`; aggregate seven-receipt listing SHA-256: `d37f976ace2661f9abeaabb163149fdba62ac1446aaa7b9b698b6aa61aeb293c`.

**OBSERVED:** Per-attempt streamed receipts reported 539,681 prompt tokens (267,776 cached), 1,069 output, and 581 reasoning. The two persisted compaction entries recorded another 106,656 uncached prompt, 3,268 output, and 2,586 reasoning tokens that were absent from `worker:glance`. Combined provider-visible usage was therefore 646,337 prompt (267,776 cached; 378,561 uncached), 4,337 output, and 3,167 reasoning. Pi's summed `cost` value `0.08627212` is an estimated API-price equivalent, not billed spend under subscription OAuth.

**OBSERVED:** The first task request after each Pi compaction had 0% cached input, while small resumed requests before the next compaction reached 98.4–98.8% cached. Repeated compaction also required a valid cut point: Pi refused a no-op second summary while the retained 53K-token user turn was atomic and later turns totaled less than `keepRecentTokens`. These are desirable semantics, but the cache cliff and hidden summarization usage must be included in long-worker accounting.
...
- **OBSERVED:** A broad read-only Pi source-review brief timed out after 15 minutes despite 96.6% cached prompt input. Direct bounded source inspection was faster. Cheap Luna capacity does not make broad briefs or avoidable rediscovery free in elapsed time.

## Consequence

**DERIVED:** Stop expanding the harness survey. Operationalize Pi behind Stensibly, retain Codex as the visible commander, separate Sol/Luna cached/uncached/output/reasoning accounting, add compaction-entry usage to worker totals, and supervise fleets through bounded glances rather than worker narration. Reopen a comparison only for a concrete discriminator such as demonstrated long-session rediscovery or forced-kill resume loss.
